...
class TestDycifer(unittest.TestCase):
    ...
    def test_readSignals(self):
        ...
        """
        # save the signals
        sdf = DataFrame({"signal 1": s1, "signal 2": s2, "signal 3": s3, "time [s]":t, "signal 4": s4})
        print(sdf.head(5))
        ...
        self.assertTrue(np.allclose(signals.reset_index().values, sdf[signals.reset_index().columns].values))
        file_path_fooling = "./resources/data/fool_test_signals.csv"
        print()
        sdf_fooling = DataFrame({"signal 1": s1, "signal 2": s2, "signal 3": s3, "notime [A]":t, "signal 4": s4})
    ...
    def test_adcDynamicEval(self):
        ...
        spectrum, signal_power, dc_power, sfdr, thd, snr, sndr, enob = adcDynamicEval(signals, fs/5, n_bits=res, signal_span_factor=0.002) # 0.2 % of power spectral density leakage
        plt.stem(spectrum[spectrum.index>0].index, spectrum["power_db"][spectrum.index>0].values, bottom=spectrum["power_db"].min())
