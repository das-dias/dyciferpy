...
def plotPrettyFFT(
    ...
    for c, ls, f in zip(colours, line_styles, signal_freq):
        plt.axvline(f, ymin=min(power[1:]) ,color=c, linestyle=ls, linewidth=3, label=f"Fundamnetal Harmonic @ {f/1e9}G Hz")
    plt.axhline(max(power[1:]), color='k', linestyle='--', linewidth=3, label=f"Fundamental Harmonic Power = {max(power[1:]):.2f} (dB)")
    #pdb.set_trace()
    plt.grid()
    plt.title(title)
    plt.xlabel(xlabel)
