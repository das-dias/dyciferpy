...
__cmds = {
    "-ms": ("mixed-signals", "Mixed Signals dynamic performance evaluation", mixedSignalsDynamicEval),
    "-a":   ("analog", "Analog integrated circuits dynamic performance evaluation", analogDynamicEval)
}
# define the arguments of each subparser
__cmd_args = {
    "-ms":{
        "-adc": ("--analog-to-digital", "Analog-to-Digital Converter dynamic performance evaluation", "", bool, "opt" ),
        "-dac": ("--digital-to-analog", "Digital-to-Analog Converter dynamic performance evaluation", "", bool, "opt" ),
        "-sda":  ("--sigma-delta-adc", "Sigma-Delta Analog-to-Digital Converter dynamic performance evaluation", "", bool, "opt"),
        "-sdd":  ("--sigma-delta-dac", "Sigma-Delta Digital-to-Analog Converter dynamic performance evaluation", "", bool, "opt"),
        "-fs":  ("--sampling-frequency", "Sampling frequency of the parsed signals", "<frequency>", str, ""), # obrigatório especificar a frequência de amostragem
    ...
    "-a":{
        ...
        "-scc":("--switch-capacitor-circuit", "Switch Capacitor Circuit dynamic performance evaluation", "", bool, "opt"),
        "-amp":("--amplifier", "Amplifier dynamic performance evaluation", "", bool, "opt")
    },
    "all":{
        "-s":("--signals","Specify the input path of the file containing the time-series signals to analyse", "<filepath>", str,""),
        "-o":("--output-dir","Specify the output directory for the resulting output files", "<directory-path>", str, "opt"),
        "-p":("--plot","Boolean flag to indicate the programme to plot or not the resulting analysis graphs", "", bool, "opt"),
        "-gt":("--generate-table","Boolean flag to indicate the programme to generate a markdown and a latex table with the resulting analysis data", "", bool, "opt")
    }
}
...
def setupParser(cmds: dict, args:dict ) -> argparse.ArgumentParser:
    ...
    """
    parser = argparse.ArgumentParser(description='DYCIFER - Dynamic Circuits Performance Evaluation Tool')
    parser.add_argument("-v", "--version", action="version", version=f"DYCIFER v{__version__}")
    ...
    for cmd, (cmd_name, cmd_desc, cmd_callback) in cmds.items():
        ...
        for arg, (arg_literal, arg_desc, arg_metavar, arg_type, arg_opt) in args[cmd].items():
        ...
        if bool(__cmd_args.get("all")):
            for arg, (arg_literal, arg_desc, arg_metavar, arg_type, arg_opt) in __cmd_args["all"].items():
                if arg_type in [bool, None]:
                    subparser.add_argument(
                        arg, 
                        arg_literal, 
